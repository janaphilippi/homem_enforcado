...
def rand_word():
        with open("Lista-de-Palavras.txt", "rt") as f:
                bank = f.readlines()
                bank_list = [x.lower() for x in bank]
        return bank_list[random.randint(0,len(bank_list))].strip()
...
def main():
    ...
    while terminou == False:
        entrada = list(str(input('Digite uma letra: ')).lower())
        #print(entrada)
